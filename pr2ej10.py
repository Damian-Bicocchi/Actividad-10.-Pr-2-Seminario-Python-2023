def punto_a_relacionar(nombre,n1,n2):
    ...
    nombres_limpios = map(lambda x: x.capitalize() ,nombres_limpios)

    tuplas_notas = zip(n1,n2)

    diccionario_intermedio = dict(zip(nombres_limpios,tuplas_notas))
...
def punto_c_promedio_general(un_diccionario):
    ...
    prom_todos = map(calc_prom, un_diccionario.values())

    prom = sum(prom_todos) / len(un_diccionario) if len(un_diccionario) != 0 else 0
...
#Punto A
estructura_A_todos = punto_a_relacionar(nombres,notas_1,notas_2)
for key,value in estructura_A_todos.items():
    print(f"{key} : {value}")
    print("-"*40)
#Punto B
